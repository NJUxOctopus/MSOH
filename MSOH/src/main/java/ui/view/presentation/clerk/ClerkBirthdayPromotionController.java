 ...
 */
public class ClerkBirthdayPromotionController implements ControlledStage {

    private StageController stageController;

    private String resource = "clerk/ClerkBirthdayPromotion.fxml";
    ...
    private void confirmCreate() throws IOException, ClassNotFoundException {
        ...
        if (confirmButton.getText().equals("制定")) {
            ResultMessage resultMessage = editPromotion.addHotelPromotion(new PromotionVO(clerkName, time, "生日特惠", MemberType.NORMAL
                    , targetHotel, startTime, endTime, discount, 0, PromotionType.HotelPromotion_Birthday));
            ...
            ResultMessage resultMessage = editPromotion.modifyHotelPromotion(new PromotionVO(clerkName, time, "生日特惠", MemberType.NORMAL
                    , targetHotel, startTime, endTime, discount, 0, PromotionType.HotelPromotion_Birthday));
            if (resultMessage.equals(ResultMessage.Blank)) {
                this.returnMessage("信息未填写完整！");

 ...
 */
public class MarketerVIPPromotionController implements ControlledStage {
    ...
    private String resource = "marketer/MarketerVIPPromotion.fxml";
    ...
    public void initial(String ID) throws RemoteException {
        processMember = new ProcessMemberController();
        maxLevel = processMember.getMemberLevel().num;
     ...
     */
    @FXML
    private void addMemberLevel() {
        int level = Integer.parseInt(memberLevelLabel.getText());
        if (level != maxLevel) {
            memberLevelLabel.setText(String.valueOf((level + 1)));
        }
    }
    ...
    private void confirmCreate() throws IOException, ClassNotFoundException {
        ...
        int memberLevel = Integer.parseInt(memberLevelLabel.getText());
        ...
        if (confirmButton.getText().equals("制定")) {
            ResultMessage resultMessage = editPromotion.addWebPromotion(new PromotionVO(marketerName, time, name, MemberType.NORMAL
                    , targetArea, memberLevel, startTime, endTime, discount, 0, PromotionType.WebPromotion_VIP));
            ...
            ResultMessage resultMessage = editPromotion.modifyWebPromotion(new PromotionVO(marketerName, time, name, MemberType.NORMAL
                    , targetArea, memberLevel, startTime, endTime, discount, 0, PromotionType.WebPromotion_VIP));
            if (resultMessage.equals(ResultMessage.Blank)) {
                this.returnMessage("信息未填写完整！");
